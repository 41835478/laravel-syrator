...
@endif
<form method="post" action="{{ _route('admin:mygz.material.cat.update', $catalog->id) }}" accept-charset="utf-8">
	{!! method_field('put') !!}
    {!! csrf_field() !!}
		...
		<div class="tab-content">
			<div class="tab-pane active" id="tab_1">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="name" autocomplete="off" value="{{ old('name', isset($catalog) ? $catalog->name : null) }}" placeholder="">
				</div>
				<div class="form-group">
    				<label>上级分类<small class="text-red">*</small></label> 
    				<div class="input-group">
                        <input class="form-control" type="text" name="parent" id="parent" value="{{ old('pid', isset($catalog) ? $catalog->pid : null) }}">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="sort_num" autocomplete="off" value="{{ old('sort_num', isset($catalog) ? $catalog->sort_num : null) }}" placeholder="">
				</div>
				<div class="form-group">
					<label>是否展示<small class="text-red">*</small></label>
                    <div class="input-group">
                        <input type="radio" name="is_show" value="1" {{ ($catalog->is_show === 1) ? 'checked' : '' }}>
                        <label class="choice" for="radiogroup">是</label>
                        <input type="radio" name="is_show" value="0" {{ ($catalog->is_show === 0) ? 'checked' : '' }}>
                        <label class="choice" for="radiogroup">否</label>
                  	</div>
				</div>				
				<div class="form-group">
					<label>关键字</label> 
					<input type="text" class="form-control" name="keywords" autocomplete="off" value="{{ old('keywords', isset($catalog) ? $catalog->keywords : null) }}" placeholder="">
				</div>
				<div class="form-group">
					<label>描述</label>
